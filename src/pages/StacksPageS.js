...
// Stable constant prevents dependency churn
const DEFAULT_FORM_STATE_S = Object.freeze({
  title: '',
  description: '',
  image: '',
  mainPorints: '',
...
const PageStylesS = () => {
  const css = `
    ...
    .stack-card { background: var(--background-white); border: 1px solid var(--border-color); border-radius: 16px; display: flex; flex-direction: column; box-shadow: 0 4px 20px rgba(0,0,0,0.05); transition: all 0.3s ease; }
    ...
    .modal-content { background: var(--background-white); padding: 40px; border-radius: 16px; width: 90%; max-width: 600px; }
...
const HeaderS = ({ onAddClick }) => (
  React.createElement('header', null,
    React.createElement('nav', { className: 'container' },
      React.createElement(LinkS, { to: '/homepage', className: 'logo' }, 'ELearning'),
...
const StackFormModalS = ({ show, onClose, stack, onSave }) => {
  const [formData, setFormData] = useStateS(() => ({ ...DEFAULT_FORM_STATE_S }));

  useEffectS(() => {
    if (show) setFormData(stack ? { ...stack } : { ...DEFAULT_FORM_STATE_S });
  }, [show, stack]);
  ...
  const handleSubmit = async (e) => {
    ...
    const url = isUpdating
      ? `https://sewedylearn.runasp.net/api/CoreStackSenior/UpdateCoreS/${stack.id}`
      : 'https://sewedylearn.runasp.net/api/CoreStackSenior/AddCoreS';
    ...
    React.createElement('div', { className: 'modal-overlay visible' },
      React.createElement('div', { className: 'modal-content' },
        React.createElement('div', { className: 'modal-header' },
          ...
          React.createElement('button', { onClick: onClose, className: 'close-button' }, React.createElement(CloseIconS))
        ),
  ...
  React.createElement('div', { className: 'stack-card' },
    ...
    React.createElement('div', { className: 'stack-card-content' },
      ...
      React.createElement('div', { className: 'stack-card-actions' },
        React.createElement(LinkS, { to: `/videos/senior/${stack.id}`, className: 'button primary' }, 'View Videos'),
...
const StacksPageS = () => {
  const [stacks, setStacks] = useStateS([]);
  const [isLoading, setIsLoading] = useStateS(true);
  const [error, setError] = useStateS(null);
  const [isModalOpen, setIsModalOpen] = useStateS(false);
  const [editingStack, setEditingStack] = useStateS(null);
  ...
  const renderContent = () => {
    ...
    return React.createElement('div', { className: 'stacks-grid' }, stacks.map((stack) => (
      React.createElement(StackCardS, { key: stack.id, stack, onEdit: handleOpenEditModal, onDelete: handleDelete })
    )));
  };

  return (
    React.createElement('div', { className: 'stacks-page-layout' },
      React.createElement(PageStylesS, null),
